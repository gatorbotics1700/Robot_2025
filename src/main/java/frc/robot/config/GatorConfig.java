    ...
    public static GatorConfig getConfig() {
        ...
        if (edu.wpi.first.wpilibj.RobotBase.isReal()) {
            ...
            try {
                ...
                System.err.println("Failed to read RoboRIO serial number. Defaulting to manta config.");
                return getMantaConfig();
            }

            switch (serialNum) {
                case "031b1d4b": // Manta
                    return getMantaConfig();
                case "nemo_serial": // TODO: Replace with actual serial
                    return getNemoConfig();
                case "hulk_serial": // TODO: Replace with actual serial
                    return getHulkConfig();
                case "dory_serial": // TODO: Replace with actual serial
                    return getDoryConfig();
                default:
                    System.err.println("Unknown robot serial number: " + serialNum + ". Defaulting to manta config.");
                    return getMantaConfig();
            }
        } else {
            // We're in simulation/test environment
            System.out.println("Running in simulation mode. Using manta config.");
            return getMantaConfig();
        }
    }

    private static GatorConfig getMantaConfig() {
